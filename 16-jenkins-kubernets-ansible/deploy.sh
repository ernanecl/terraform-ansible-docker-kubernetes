...
echo "$(/usr/bin/terraform output | grep public_dns | awk '{print $2;exit}')" | sed -e "s/\",//g" >> ../1-ansible/hosts # captura output faz split de espaco e replace de ",
...
echo $"Agora somente abrir a URL: http://$(/usr/bin/terraform output | grep public | awk '{print $2;exit}'):8080" | sed -e "s/\",//g"
...
ID_MAQUINA=$(/usr/bin/terraform output | grep id | awk '{print $2;exit}')
echo ${ID_MAQUINA/\",/}
